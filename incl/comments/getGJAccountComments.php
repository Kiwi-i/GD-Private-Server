...
$query = "SELECT comment, userID, likes, spam, commentID, timestamp FROM acccomments WHERE userID = :userID ORDER BY timeStamp DESC LIMIT 10 OFFSET $commentpage";
...
foreach($result as &$comment1) {
	if($comment1["commentID"]!=""){
		...
		$commentstring .= "2~".$comment1["comment"]."~3~".$comment1["userID"]."~4~".$comment1["likes"]."~5~0~7~".$comment1["spam"]."~9~"."$uploadDate"."~6~".$comment1["commentID"]."|";
	}
}
