	...
	if(strlen($username) < 3){
		...
		if($registred_users > 0){
			...
			if($password != $repeat_password){
				...
				$query2 = $db->prepare("INSERT INTO accounts (userName, password, email, registerDate, activated, gjp2)
				VALUES (:userName, :password, :email, :time, :activated, :gjp2)");
				$query2->execute([':userName' => $username, ':password' => $hashpass, ':email' => $email,':time' => time(), ':activated' => $preactivateAccounts ? 1 : 0, ':gjp2' => GeneratePass::GJP2hash($password)]);
